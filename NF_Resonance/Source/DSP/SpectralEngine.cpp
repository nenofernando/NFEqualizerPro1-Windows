...
float SpectralEngine::processOne(Chan& s,float x,int channelIndex){ long long t=s.t; int out=(int)(t%ringSize); float nrm=s.norm[(size_t)out];
 ...
 float y=ok?s.ola[(size_t)out]/nrm:0.0f; s.ola[(size_t)out]=0;s.norm[(size_t)out]=0; s.history[(size_t)s.histPos]=x;s.histPos=(s.histPos+1)%fftSize; float tf=s.trans.process(x,params.transient); ++s.t;
