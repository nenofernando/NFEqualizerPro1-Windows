#!/bin/bash
# Builds the full "final product" macOS installer for NF Pro Clipper:
# a welcome/choices/conclusion .pkg (VST3 + AU, user picks which) wrapped in a DMG.
# Requires the VST3 and AU targets to already be built (Release config, build-release dir).
set -euo pipefail

PLUGIN_NAME="NF Pro Clipper"
VERSION="1.0.0"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
INSTALLER_DIR="$ROOT_DIR/installer/mac"
MANUAL_DIR="$ROOT_DIR/manual"
ARTEFACTS_DIR="$ROOT_DIR/build-release/NFProClipper_artefacts/Release"
VST3_PATH="$ARTEFACTS_DIR/VST3/$PLUGIN_NAME.vst3"
AU_PATH="$ARTEFACTS_DIR/AU/$PLUGIN_NAME.component"
MANUAL_EN_PATH="$MANUAL_DIR/NF_Pro_Clipper_Manual_EN.pdf"
MANUAL_PT_PATH="$MANUAL_DIR/NF_Pro_Clipper_Manual_PT.pdf"
OUT_DIR="$ROOT_DIR/$PLUGIN_NAME - Download Package"
FINAL_PKG_NAME="NF Pro Clipper Installer.pkg"
...
for path in "$VST3_PATH" "$AU_PATH" "$MANUAL_EN_PATH" "$MANUAL_PT_PATH"; do
    if [ ! -e "$path" ]; then
        ...
        echo "Build both plugin formats first with: cmake --build \"$ROOT_DIR/build-release\" --config Release" >&2
...
pkgbuild \
    ...
    --identifier "com.nenofernando.nfproclipper.vst3" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFProClipper-VST3.pkg"

pkgbuild \
    --root "$WORK_DIR/au-root" \
    --identifier "com.nenofernando.nfproclipper.au" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFProClipper-AU.pkg"

pkgbuild \
    --root "$WORK_DIR/docs-root" \
    --identifier "com.nenofernando.nfproclipper.docs" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFProClipper-Docs.pkg"
