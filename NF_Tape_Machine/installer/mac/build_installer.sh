#!/bin/bash
# Builds the full "final product" macOS installer for NF Tape Machine:
...
PLUGIN_NAME="NF Tape Machine"
VERSION="0.1.0"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
INSTALLER_DIR="$ROOT_DIR/installer/mac"
MANUAL_DIR="$ROOT_DIR/manual"
ARTEFACTS_DIR="$ROOT_DIR/build-release/NFTapeMachine_artefacts/Release"
VST3_PATH="$ARTEFACTS_DIR/VST3/$PLUGIN_NAME.vst3"
AU_PATH="$ARTEFACTS_DIR/AU/$PLUGIN_NAME.component"
MANUAL_EN_PATH="$MANUAL_DIR/NF_Tape_Machine_Manual_EN.pdf"
MANUAL_PT_PATH="$MANUAL_DIR/NF_Tape_Machine_Manual_PT.pdf"
OUT_DIR="$ROOT_DIR/$PLUGIN_NAME - Download Package"
FINAL_PKG_NAME="NF Tape Machine Installer.pkg"
...
pkgbuild \
    ...
    --identifier "com.nfaudio.nftapemachine.vst3" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFTapeMachine-VST3.pkg"

pkgbuild \
    --root "$WORK_DIR/au-root" \
    --identifier "com.nfaudio.nftapemachine.au" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFTapeMachine-AU.pkg"

pkgbuild \
    --root "$WORK_DIR/docs-root" \
    --identifier "com.nfaudio.nftapemachine.docs" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFTapeMachine-Docs.pkg"
