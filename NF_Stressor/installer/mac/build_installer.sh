#!/bin/bash
# Builds the full "final product" macOS installer for NF - Stressor:
...
PLUGIN_NAME="NF - Stressor"
VERSION="1.0.0"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
INSTALLER_DIR="$ROOT_DIR/installer/mac"
MANUAL_DIR="$ROOT_DIR/manual"
ARTEFACTS_DIR="$ROOT_DIR/build/NFStressor_artefacts"
VST3_PATH="$ARTEFACTS_DIR/VST3/$PLUGIN_NAME.vst3"
AU_PATH="$ARTEFACTS_DIR/AU/$PLUGIN_NAME.component"
MANUAL_EN_PATH="$MANUAL_DIR/NF_Stressor_Manual_EN.pdf"
MANUAL_PT_PATH="$MANUAL_DIR/NF_Stressor_Manual_PT.pdf"
OUT_DIR="$ROOT_DIR/$PLUGIN_NAME - Download Package"
FINAL_PKG_NAME="NF - Stressor Installer.pkg"
...
pkgbuild \
    ...
    --identifier "com.nfaudiotools.stressor.vst3" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFStressor-VST3.pkg"

pkgbuild \
    --root "$WORK_DIR/au-root" \
    --identifier "com.nfaudiotools.stressor.au" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFStressor-AU.pkg"

pkgbuild \
    --root "$WORK_DIR/docs-root" \
    --identifier "com.nfaudiotools.stressor.docs" \
    --version "$VERSION" \
    --install-location "/" \
    "$PKG_DIR/NFStressor-Docs.pkg"
